...
async function requestAgentCopy(
  ...
  let fallbackValue: Partial<AIModalResponse> | null = null;
  ...
  for (let attempt = 0; attempt < liveAgentRequestAttempts; attempt += 1) {
    ...
    const value = (await response.json()) as Partial<AIModalResponse>;

    if (value.source === "openrouter") {
      return value;
    }
...
function MatchingAgentPanel({
  ...
  return (
    <section className="panel matching-agent-panel" aria-labelledby="agent-panel-title">
      <div className="match-panel-heading">
        <div>
          <h2 id="agent-panel-title">FoodLoop Matching AI</h2>
...
function MatchSourceBadge({
  ...
  return (
    ...
    >
      {isLive ? "FoodLoop AI recommendation" : "Fallback demo data"}
    </span>
  );
}
      ...
      >
        <div className="ai-modal-header">
          <div>
            ...
            <h2 id="ai-modal-title">
              {response ? response.title : "Generating FoodLoop AI draft"}
            </h2>
          </div>
          <button
            type="button"
        ...
        {!response ? (
          <div className="modal-loading" role="status" aria-live="polite">
            ...
            <div>
              <strong>Generating FoodLoop AI recommendation</strong>
            ...
            <div className="modal-source-note">
              ...
              <span>
                {response.source === "openrouter"
                  ? `Generated as a FoodLoop AI recommendation${response.model ? ` (model: ${response.model})` : ""}.`
